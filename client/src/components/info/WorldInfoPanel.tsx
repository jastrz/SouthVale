...
export function WorldInfoPanel() {
  ...
  return (
    <div className="flex w-full max-w-sm flex-col gap-2 rounded-xl bg-slate-800/80 px-4 py-3 text-xs text-slate-300">
      ...
      {data.previous && data.previous.winners.length > 0 && (
        <div className="flex flex-col gap-1">
          ...
          {data.previous.winners.map((w, i) => (
            <div key={w.username} className="flex items-center gap-2">
              <span
      ...
      {data.bestEver && (
        <p className="border-t border-slate-700 pt-2 text-center text-[11px] text-slate-400">
